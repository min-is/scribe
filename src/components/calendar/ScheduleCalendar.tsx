...
function ShiftCard({ shift }: ShiftCardProps) {
  ...
  const timeStr = formatShiftTime(shift.startTime, shift.endTime);
      ...
      <div className="mt-1">
        {shift.scribe && (
          <div className="text-sm font-medium text-zinc-900 dark:text-zinc-100">
            {shift.scribe.standardizedName || shift.scribe.name}
          </div>
        )}
        {shift.provider && (
          <div className="text-xs text-zinc-600 dark:text-zinc-400">
            with {shift.provider.name}
          </div>
        )}
      </div>
    </div>
    ...
    const fetchMonthShifts = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/shifts/range?startDate=${startDate}&endDate=${endDate}`
        );
        ...
        if (data.success && data.data) {
          ...
          data.data.forEach((shift: ShiftWithRelations) => {
  ...
  useEffect(() => {
    ...
    const fetchDailySchedule = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/shifts/daily?date=${dateStr}`);
