...
class ScheduleParser:
    ...
    def validate_html_structure(html_content: str) -> Tuple[bool, str]:
        ...
        if len(html_content) < 100:
            return False, "HTML content too short (likely invalid)"

        # Check for expected HTML structure markers
        expected_markers = [
            "vertical-align:text-top",  # Day cell style
            "font-size:12px"            # Day number style
        ]
        ...
        if missing_markers:
            return False, f"Missing expected HTML markers: {', '.join(missing_markers)}"

        # Check for basic HTML tags
        if not any(tag in html_content.lower() for tag in ['<td', '<div', '<span']):
            return False, "Missing basic HTML structure (no td/div/span tags)"

        # Check for month/year header
        if not any(x in html_content for x in ['font-weight:bold', 'font-size:16px']):
            return False, "Missing calendar header (month/year)"

        return True, ""

    def parse_calendar(self, html_content: str, site_name: str = "") -> List[Dict[str, str]]:
        """
        Parse calendar HTML into structured data with validation.
        ...
        Raises:
            ValueError: If HTML structure validation fails
        """
        # Validate HTML structure first
        is_valid, error_msg = self.validate_html_structure(html_content)
        if not is_valid:
