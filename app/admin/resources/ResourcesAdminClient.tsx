...
type ResourcesAdminClientProps = {
  sections: ResourceSection[];
};

type EditMode =
  | { type: 'none' }
  | { type: 'section'; section?: ResourceSection }
...
export default function ResourcesAdminClient({
  ...
  const handleDeleteSection = async (section: ResourceSection) => {
    const articleCount = section.children?.length || 0;
    const confirmMsg =
      articleCount > 0
        ? `Are you sure you want to delete "${section.title}" and its ${articleCount} article(s)?`
        : `Are you sure you want to delete "${section.title}"?`;

    if (!confirm(confirmMsg)) return;

    const result = await deleteResourceSection(section.id);
  ...
  const handleSubmitSection = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result =
        editMode.type === 'section' && editMode.section
          ? await updateResourceSection(editMode.section.id, data)
          : await createResourceSection(data);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-dim mt-1">
              Manage resource sections and articles with rich content
            </p>
          </div>
          {editMode.type === 'none' && (
            <button
          ...
          <div className="space-y-6">
            {initialSections.length === 0 ? (
              <div className="text-center py-12 bg-medium border border-main rounded-lg">
                ...
                <p className="text-dim text-lg">No sections yet</p>
